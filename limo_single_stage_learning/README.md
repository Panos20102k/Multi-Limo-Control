...
After bringing up `limo_gazebo`, build and run. The experiment launch starts a
velocity-enabled EKF by default:
...
For a real LIMO, use wall time:

```bash
ros2 launch limo_single_stage_learning single_stage_experiment.launch.py use_sim_time:=false
```

If the robot bringup already starts an equivalently configured EKF, prevent a
duplicate node with `start_ekf:=false`.
...
The learner and PT1 filter use `/limo_1/odometry/filtered` by default. The
included `ekf_velocity.yaml` explicitly fuses wheel-odometry forward velocity
(`vx`) and publishes at 100 Hz to match the learner. Gazebo ground truth is
only a validation signal and is not required by the controller, filter, or
learner. The input can still be remapped with the `velocity_topic` parameter.
