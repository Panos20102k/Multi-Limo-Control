"""ROS 2 node for the four-stage online learning experiment."""
...
from .core import FourStageLearner
...
class LearningNode(Node):
    def __init__(self):
        ...
        self.learner = FourStageLearner(
            dt=p('control_period'), interval=p('rls_interval'),
            policy_window=p('policy_window'), learning_duration=p('learning_duration'),
            stage_duration=p('stage_duration'), q=p('q'), r=p('r'), rho=p('rho'),
            forgetting=p('rls_forgetting'), covariance_scale=p('covariance_scale'),
            policy_blend=p('policy_blend'), initial_slope=p('initial_policy_slope'),
            initial_intercept=p('initial_policy_intercept'),
            exploration_amplitude=p('exploration_amplitude'),
            exploration_decay=p('exploration_decay'), model_tau=p('validation_model_tau'),
        ...
        self.scalar_pubs = {name: self.create_publisher(Float64, '~/' + name, 20)
                            for name in names}
        self.policy_pub = self.create_publisher(Float64MultiArray, '~/policy', 20)
        self.theta_pub = self.create_publisher(Float64MultiArray, '~/theta', 20)
    ...
    def _tick(self):
        ...
        self.stage_pub.publish(UInt8(data=sample.stage))
        ...
        if self.learner.step_index >= self.learner.total_steps:
            ...
            self.finished = True
            self.get_logger().info('Four-stage experiment complete; zero command published')
